...
print("CH Corporations - Prateleira dinâmica (revisão: admin + dono + sync robusta)")
...
local function criarCaixaVisual(slotPart, nomeDaPeca, cliques, isPremium, isSuspensao, isECU, isGalao, isSpray)
	...
	elseif isECU then
		...
		if findECU then
			...
			warn("[Prateleira] ECU visual em falta: CaixaFalsa" .. nomeDaPeca)
			return
		end
	...
	for _, record in ipairs(invJogador:GetChildren()) do
		if record.Name == nomeDoItem or record.Name == nomeUnderline or record.Name == nomeEspaco then
			if not valorOpcional or record.Value == valorOpcional then
				...
				sucessoAoApagar = true
				break
			end
		end
	end

	if not sucessoAoApagar then
		warn("[Prateleira] Anti-dupe: recibo '" .. nomeDoItem .. "' não encontrado para remover.")
	end
end
